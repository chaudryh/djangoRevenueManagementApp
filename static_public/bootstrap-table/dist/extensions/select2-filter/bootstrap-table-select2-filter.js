...
!function ($) {
    ...
    if (enableFilter) {
      ...
      if ($inputs.length > 0) {
        ...
        $inputs.off('keyup').on('keyup', function (event) {
          ...
          if (value) {
            that.filterColumnsPartial[field] = value;
          } else {
        ...
        // onColumnSearch: Change to Enter Key
        $inputs.off('keypress').on('keypress', function (event) {
          if (event.key === 'Enter') {
            that.onColumnSearch(event);
          }
        });
        // Remove the sort listener on the head in bootstrap-table.js
  ...
  BootstrapTable.prototype.onColumnSearch = function (event) {
    ...
    // onColumnSearch: set isColumnSearch
