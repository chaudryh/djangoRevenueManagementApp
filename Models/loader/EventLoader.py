...
class EventLoadProcessor:
    ...
    @classmethod
    def event_packages_price_change_init(cls):
        ...
        price_records = price_records.where((pd.notna(price_records)), None)
        ...
        for center_obj in center_objs:
            for product_id in product_ids:
                ...
                price = price_records.loc[
                    (price_records['center_id'] == center_id) & (price_records['product_id'] == product_id),
                    'price'
                ]
    ...
    @classmethod
    def event_bowling_price_change_init(cls):
        ...
        center_records['bowling_event_tier'] = center_records['bowling_event_tier'].apply(
    ...
    @classmethod
    def event_shoe_price_change_init(cls):
        ...
        if not product_ids:
            return
        last_price_records = DataDAO.LastPrice.get_last_price(['115'], centers, UDatetime.now_local().date())
...
if __name__ == "__main__":
    loader = input('Please give the loader type('
                   ...
                   '6:init event bowling price change; '
                   '7:init event shoe price change; '
                   '8:init event packages price change; '
                   '):')
    if int(loader) == 1:
        ...
        EventLoadProcessor.event_bowling_price_change_init()
    elif int(loader) == 7:
        EventLoadProcessor.event_shoe_price_change_init()
    elif int(loader) == 8:
        EventLoadProcessor.event_packages_price_change_init()
